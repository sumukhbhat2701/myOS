...
void print(char* s)
{
	...
	// As we need alternate blocks, we use unsigned short which is 2 bytes instead of 4 incase of unsigned int. screen+i by 2*i bytes incase of short, which is 4*i bytes incase of int. 
	// As we need only 2nd byte of every 2 bytes of unsigned short, we 'and' the mem location with 2 bytes with 0xFF00 and 'or' with 1 byte of character to be written.  
